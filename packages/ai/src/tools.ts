...
export const renderMetricCard = tool({
	...
	inputSchema: z.object({
		title: z
			.string()
			.describe("The label for the metric (e.g., 'Total Users')."),
		...
		icon: z
			.string()
			.optional()
			.describe("Icon name from Lucide (e.g., 'users', 'dollar-sign')."),
	}),
...
export const renderUserForm = tool({
	description:
		"Render a form to capture user input for creating or editing data.",
	inputSchema: z.object({
		title: z.string(),
		fields: z.array(
			z.object({
...
export const renderToastStack = tool({
	...
	inputSchema: z.object({
		...
		description: z
			.string()
			.optional()
			.describe("Optional description for the notifications."),
		toasts: z
			.array(
				z.object({
					id: z.string(),
...
export const renderAccordion = tool({
	...
	inputSchema: z.object({
		...
		description: z
			.string()
			.optional()
			.describe("A brief description of the accordion."),
		items: z
			.array(
				z.object({
					id: z
						.string()
						.describe("A unique identifier for the accordion item."),
		...
		defaultValue: z
			.string()
			.optional()
			.describe("The ID of the item that should be open by default."),
		collapsible: z
			.boolean()
			.optional()
			.describe(
...
export const renderCrudDataTable = tool({
	description:
		"Render a table with Create, Read, Update, and Delete capabilities.",
	inputSchema: z.object({
		title: z.string(),
	...
	inputSchema: z.object({
		...
		steps: z.array(
			z.object({
				...
				icon: z
					.enum(["order", "processing", "shipping", "delivered"])
					.optional(),
			}),
		),
	}),
