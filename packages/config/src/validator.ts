...
export class ConfigValidator {
	...
	static validatePageReferences(config: SynthConfig): string[] {
		...
		for (const page of config.pages) {
			...
			if (!ConfigValidator.tableExists(config, page.dataSource.table)) {
				errors.push(
			...
			for (const component of page.components) {
				if (component.type === "table") {
					...
					for (const col of component.columns) {
						if (!ConfigValidator.columnExists(table, col.field)) {
							errors.push(
					...
					for (const field of component.fields) {
						if (!ConfigValidator.columnExists(table, field.name)) {
							errors.push(
	...
	static validateAll(config: SynthConfig): {
		...
		const errors = [
			...ConfigValidator.validatePageReferences(config),
			...ConfigValidator.validateComponentReferences(config),
		];
