		...
		<Card className="rounded-2xl border-none bg-card shadow-lg">
			...
			<CardContent className="space-y-4 pb-6">
				<form className="space-y-5" onSubmit={handleSubmit}>
						...
						<div key={field.name} className="space-y-2">
							...
							{field.type === "select" ? (
								...
								<Input
									...
									className="h-10 rounded-xl border-none bg-background/50 ring-1 ring-foreground/10 transition-all focus:ring-2 focus:ring-primary/20"
									value={formData[field.name] || ""}
									onChange={(e) =>
										handleInputChange(field.name, e.target.value)
									}
					...
					<Button
						type="submit"
						className="h-11 w-full rounded-xl font-bold text-sm tracking-wide shadow-lg shadow-primary/20 transition-all hover:shadow-primary/30 hover:shadow-xl active:scale-[0.98]"
					>
						{submitLabel || "SUBMIT"}
					</Button>
				</form>
