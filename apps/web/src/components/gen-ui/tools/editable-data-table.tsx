...
export function EditableDataTable({
	...
	const handleSaveEdit = () => {
		...
		if (updatedRows[rowIndex]) {
			const oldValue = updatedRows[rowIndex][columnKey];
											...
											<TableCell key={`${row.id || rowIndex}-${column.key}`}>
												...
												editCell?.columnKey === column.key ? (
													...
													<button
														type="button"
														className={cn(
															"-mx-2 w-full rounded px-2 py-1 text-left",
															column.editable &&
																"cursor-pointer hover:bg-muted focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring",
														)}
														onClick={() =>
															handleCellClick(
																rowIndex,
