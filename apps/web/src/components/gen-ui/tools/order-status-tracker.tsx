...
export function OrderStatusTracker({ title, description, status, steps, className }: OrderStatusTrackerProps) {
    ...
    const getStepIcon = (step: { icon?: "order" | "processing" | "shipping" | "delivered" }, isActive: boolean) => {
        ...
        switch (step.icon) {
            case "order": return <Clock className={iconClass} />
            case "processing": return <Package className={iconClass} />
            case "shipping": return <Truck className={iconClass} />
            case "delivered": return <CheckCircle2 className={iconClass} />
                                ...
                                <div key={step.id} className="relative pl-10">
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex items-center">
                                            <h4 className={cn("font-medium", isActive ? "text-primary" : isCompleted ? "text-foreground" : "text-muted-foreground")}>
                                                {step.label}
                                            </h4>
                                            {isActive && (
